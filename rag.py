...
def classify_scope(query: str) -> str:
    ...
    '''
    Classifies the scope of the query using the LLM.

    :param query: Query to classify.

    :return: Scope of the query as In-scope or Out-of-scope.
    ...
    scope_template = PromptTemplate.from_template(template=scope_prompt)

    chain = scope_template | llm | StrOutputParser()

    result = chain.invoke({"query": query})
    
    return result.strip().lower()  # Normalize the output to lowercase
...
def rag_tool(query: str) -> str:
    ...
    '''
    If the query is in-scope with respect to the classify_scope function result, this tool retrieves relevant documents from the vector store and generates a response using the LLM.
    If the query is out-of-scope, it returns a default message.
    If the query is in-scope, but no relevant documents are found, we search the web for relevant information.

    :param query: Query to process.
    :return: Response generated by the LLM based on retrieved documents.
    ...
    if classify_scope(query) == "out-of-scope":
        return "This question is out of scope. Please ask about food and restaurants."
    else:
        ...
        if not docs or len(docs) == 0:
            ...
            # If no relevant documents are found, search the web
           # tavily_tool = TavilySearch(
           #     num_results=3,
           #     search_type="web"
           # )

           # tavily_raw_results = tavily_tool.run(query)
            # Context from Tavily search results
            #tavily_results = "\n\n".join(r for r in tavily_raw_results)
